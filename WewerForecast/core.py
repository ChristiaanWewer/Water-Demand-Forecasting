...
from WewerForecast.TimeSeriesDataset import TimeSeriesDataset
...
from WewerForecast import losses
...
class TimeSeriesCore():
    def __init__(
        ...
        # scale training data
        self.df_train_scaled, self.scaling_coefficients = self.scaler.transform_df(self.df_train)
        self.df_val_scaled, _ = self.scaler.transform_df(self.df_val)


        # make train and val dataset
        self.train_dataset = TimeSeriesDataset(
            dataframes=self.df_train_scaled,
    ...
    def set_prediction_data(self, df_pred):
        ...
        self.df_pred_scaled, _ = self.scaler.transform_df(self.df_val)


            
        self.pred_dataset = TimeSeriesDataset(
            dataframes=self.df_pred_scaled,
            target_feature=self.target_feature,
            historic_features=self.historic_features,
            future_features=self.future_features,
            historic_sequence_length=self.historic_sequence_length,
